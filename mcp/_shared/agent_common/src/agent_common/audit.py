...
class AuditLog:
    def __init__(self, db_path: str | Path) -> None:
        ...
        # MCPServer runs sync tool functions in a worker thread pool, so this
        # connection (created once at import time) is called from those
        # worker threads — the default same-thread check must be disabled.
    ...
    def get(self, event_id: str) -> dict[str, Any] | None:
        row = self._conn.execute(
            f"SELECT {', '.join(_COLUMNS)} FROM audit_log WHERE event_id=?",
            (event_id,),
        ).fetchone()
